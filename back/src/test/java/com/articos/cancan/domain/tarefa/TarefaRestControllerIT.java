...
public class TarefaRestControllerIT extends SuperCrudRestControllerIT<
    ...
    public void assertFindResponse(ResultActions result, Tarefa entity) {
        result.andExpect(jsonPath("$.titulo").value(entity.getTitulo()))
    ...
    @Override
    @SneakyThrows
    public void assertCreatedOrUpdatedEntity(Tarefa savedEntity, TarefaPayloadDTO sentDto) {
    ...
    @Override
    @SneakyThrows
    public void assertResultResponse(ResultActions result, TarefaPayloadDTO sentDto) {
        result.andExpect(jsonPath("$.titulo").value(sentDto.getTitulo()))
